...
def module_setup(request, device, artifact_dir):
    def module_teardown():
        ...
        device.run_ssh('cat /var/snap/{0}/current/config/abs.env > {1}/abs.env.log'.format(APP, TMP_DIR), throw=False)
...
def test_abs_listening(device):
    device.run_ssh('test -S /var/snap/audiobookshelf/current/audiobookshelf.sock', retries=20)
