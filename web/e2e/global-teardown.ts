...
export default async function globalTeardown () {
  ...
  const journal = ssh('journalctl -u snap.audiobookshelf.abs --no-pager | tail -800', { throw: false })
  fs.writeFileSync(path.join(artifactRoot, `abs.${project}.journal.log`), journal)
  collectArtifacts()
}
